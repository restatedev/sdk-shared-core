...
// Modifications: emits calls to `proto::message_*` helpers (which dispatch
// on our `proto::Message` trait and accept `RestateBufMut` / `RestateBuf`
// buffers) instead of `prost::encoding::message::*`.
...
impl Field {
        ...
        match self.label {
            Label::Optional => quote! {
                if let ::core::option::Option::Some(ref msg) = #ident {
                    #proto::message_encode(#tag, msg, buf);
                }
            },
            Label::Required => quote! {
                #proto::message_encode(#tag, &#ident, buf);
            },
    ...
    pub fn merge(&self, ident: TokenStream) -> TokenStream {
        ...
        match self.label {
            ...
            Label::Required => quote! {
                #proto::message_merge(wire_type, #ident, buf, ctx)
            },
            Label::Repeated => quote! {
                #proto::message_merge_repeated(wire_type, #ident, buf, ctx)
            },
        }
    }
    ...
    pub fn encoded_len(&self, ident: TokenStream) -> TokenStream {
        ...
        match self.label {
            Label::Optional => quote! {
                #ident.as_ref().map_or(0, |msg| #proto::message_encoded_len(#tag, msg))
            },
            Label::Required => quote! {
                #proto::message_encoded_len(#tag, &#ident)
            },
            Label::Repeated => quote! {
                #proto::message_encoded_len_repeated(#tag, &#ident)
            },
        }
    }
