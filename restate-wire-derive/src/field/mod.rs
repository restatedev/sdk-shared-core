...
// Modifications: emit `restate_wire::Message` impls with `RestateBufMut` /
// `RestateBuf` bounds, plus support for the new `bytes = "buffer"` field type.
...
#[derive(Clone)]
pub enum Field {
    /// A scalar field (including `Buffer`).
    Scalar(scalar::Field),
    /// A message field.
