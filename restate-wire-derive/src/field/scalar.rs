...
// Modifications:
//   - Adds `BytesTy::Buffer` for the new `bytes = "buffer"` field type,
//     routing encode/merge/encoded_len through `proto::*_buffer`.
//   - All emitted paths are hard-coded under
//     `::restate_sdk_shared_core::proto::*`. The per-scalar-type prost
//     encoding modules (`string`, `uint32`, etc.) are re-exported from
//     `proto` so we emit `proto::<module>::<fn>` for every scalar.
//   - Drops Debug emission, default-value attribute handling for bytes
//     literal etc. that aren't needed for the shared-core wire schema.
...
use syn::{parse_str, Expr, ExprLit, Ident, Lit, LitByteStr, Meta, MetaNameValue, Path};

use crate::field::{bool_attr, set_option, tag_attr, Label};
use crate::proto_path;
...
impl Field {
    ...
    pub fn encode(&self, ident: TokenStream) -> TokenStream {
        ...
        // Buffer field: always emit (no default elision), bypass kind dispatch.
        if matches!(self.ty, Ty::Bytes(BytesTy::Buffer)) {
            let tag = self.tag;
            return match self.kind {
                Kind::Plain(..) => quote! {
                    #proto::encode_buffer(#tag, &#ident, buf);
                },
                Kind::Optional(..) => quote! {
                    if let ::core::option::Option::Some(ref value) = #ident {
                        #proto::encode_buffer(#tag, value, buf);
                    }
                },
                Kind::Required(..) => quote! {
                    #proto::encode_buffer(#tag, &#ident, buf);
        ...
        let encode_fn = quote!(#proto::#module::#encode_fn);
        let tag = self.tag;
    ...
    pub fn merge(&self, ident: TokenStream) -> TokenStream {
        ...
        if matches!(self.ty, Ty::Bytes(BytesTy::Buffer)) {
            return match self.kind {
                Kind::Plain(..) | Kind::Required(..) => quote! {
                    #proto::merge_buffer(wire_type, #ident, buf, ctx)
                },
                Kind::Optional(..) => quote! {
                    #proto::merge_buffer(
                        wire_type,
                ...
                Kind::Repeated | Kind::Packed => quote! {
                    {
                        let mut item = ::core::default::Default::default();
                        let res = #proto::merge_buffer(wire_type, &mut item, buf, ctx);
                        if res.is_ok() {
                            #ident.push(item);
                        }
                        res
        ...
        let merge_fn = quote!(#proto::#module::#merge_fn);
    ...
    pub fn encoded_len(&self, ident: TokenStream) -> TokenStream {
        ...
        if matches!(self.ty, Ty::Bytes(BytesTy::Buffer)) {
            ...
            return match self.kind {
                Kind::Plain(..) | Kind::Required(..) => quote! {
                    #proto::encoded_len_buffer(#tag, &#ident)
                },
                Kind::Optional(..) => quote! {
                    #ident.as_ref().map_or(0, |value| #proto::encoded_len_buffer(#tag, value))
                },
                Kind::Repeated | Kind::Packed => quote! {
                    #ident.iter().map(|value| #proto::encoded_len_buffer(#tag, value)).sum::<usize>()
                },
            };
        }
        ...
        let encoded_len_fn = quote!(#proto::#module::#encoded_len_fn);
        let tag = self.tag;
        ...
        match self.kind {
            Kind::Plain(ref default) | Kind::Required(ref default) => {
                ...
                match self.ty {
                    ...
                    Ty::Bytes(BytesTy::Vec) | Ty::Bytes(BytesTy::Bytes) => quote!(#ident.clear()),
                    Ty::Bytes(BytesTy::Buffer) => {
                        // Reset to the type's `Default` (which is `Buffer::InMemory(Bytes::new())`).
    ...
    pub fn default(&self) -> TokenStream {
        ...
        match self.kind {
            ...
            Kind::Repeated | Kind::Packed => quote!(#proto::alloc::vec::Vec::new()),
        }
    }
}
...
pub enum BytesTy {
    Vec,
    Bytes,
    /// New variant: `#[prost(bytes = "buffer", tag = "N")]` for a field
    /// of type `restate_sdk_shared_core::Buffer`.
    Buffer,
}
...
impl DefaultValue {
    ...
    pub fn from_lit(ty: &Ty, lit: Lit) -> Result<DefaultValue, Error> {
        ...
        let default = match lit {
            ...
            Lit::ByteStr(ref lit)
                if matches!(*ty, Ty::Bytes(BytesTy::Bytes) | Ty::Bytes(BytesTy::Vec)) =>
            {
    ...
    pub fn owned(&self) -> TokenStream {
        ...
        match *self {
            DefaultValue::String(ref value) if value.is_empty() => {
                quote!(#proto::alloc::string::String::new())
            }
